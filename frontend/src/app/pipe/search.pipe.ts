...
export class SearchPipe implements PipeTransform {
  ...
  transform(courses: Course[], searchText: string, minEtcs: number, maxEtcs: number,
      ...
      return courses.filter(course => {
        ...
        if (ratingsQuantity === 0) {
          return course.name.toLowerCase().includes(searchText) &&
            course.ects >= minEtcs && course.ects <= maxEtcs && this.checkSemester(course.semester, semesters);
        }
    ...
    return courses.filter(course => {
      ...
      const ratingsQuantity = course.ratings.length;
      if (ratingsQuantity === 0) {
        return course.name.toLowerCase().includes(searchText) && course.ects >= minEtcs &&
          course.ects <= maxEtcs && this.checkSemester(course.semester, semesters);
      }
